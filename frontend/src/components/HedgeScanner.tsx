...
function PositionForm({ onSubmit, loading }: {
  onSubmit: (req: HedgeRequest) => void;
  loading: boolean;
}) {
  ...
  const [hedgeQuery, setHedgeQuery] = useState('');
  ...
  const searchMarkets = async () => {
    ...
    try {
      setSearchResults(await searchPolymarket(searchQuery));
    } catch {
      setSearchResults([]);
    } finally {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const req: HedgeRequest = {
      ...
      current_price: selectedMarket.price ?? parseFloat(entryPrice) / 100,
      position_size: parseFloat(positionSize),
      search_query: hedgeQuery || searchQuery,
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-gray-900 rounded-xl p-6 border border-gray-700 space-y-5">
        ...
        {selectedMarket ? (
          <div className="p-3 rounded-lg bg-gray-800 border border-gray-600">
            ...
            <p className="text-xs text-gray-400 mt-0.5">
              {selectedMarket.price != null ? `${(selectedMarket.price * 100).toFixed(1)}¢` : ''}
            </p>
            <button
              type="button"
          ...
          <>
            <div className="flex gap-2 mb-2">
              <input
                ...
                onKeyDown={(e) => e.key === 'Enter' && (e.preventDefault(), searchMarkets())}
      ...
      {/* Direction */}
      <div>
        <label className="block text-xs text-gray-400 uppercase tracking-wider mb-1">Your Position</label>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="block text-xs text-gray-400 uppercase tracking-wider mb-1">Entry Price (¢)</label>
          <input
        ...
        <div>
          <label className="block text-xs text-gray-400 uppercase tracking-wider mb-1">Position Size ($)</label>
          <input
      ...
      <div>
        <label className="block text-xs text-gray-400 uppercase tracking-wider mb-1">
          Hedge Search Keywords
        </label>
        <input
          value={hedgeQuery}
          onChange={(e) => setHedgeQuery(e.target.value)}
          placeholder="e.g. BTC price, crypto, bitcoin — leave blank to use market search"
          className="w-full bg-gray-700 text-white rounded px-3 py-2 text-sm border border-gray-600 focus:outline-none focus:border-indigo-500"
        />
        <p className="text-xs text-gray-500 mt-1">Keywords used to find correlated hedge candidates on Polymarket</p>
      </div>

      {/* Crypto / BL section */}
      <div className="border border-gray-700 rounded-lg p-4 space-y-3">
        <p className="text-xs text-gray-400 uppercase tracking-wider">
          Crypto Position? (optional — enables Deribit BL signal)
        </p>
        <div className="grid grid-cols-3 gap-3">
          ...
          <div>
            ...
            <input
              ...
              disabled={!asset}
              className="w-full bg-gray-700 text-white rounded px-3 py-2 text-sm border border-gray-600 focus:outline-none focus:border-indigo-500 disabled:opacity-40"
            />
          </div>
          <div>
      ...
      >
        {loading ? 'Scanning for hedges…' : 'Scan for Hedges'}
      </button>
    </form>
  );
}
